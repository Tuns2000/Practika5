...
document.getElementById('registerForm').addEventListener('submit', async (e) => {
    ...
    document.getElementById('registerMessage').textContent = result.message || 'Регистрация не удалась';
});
...
document.getElementById('fetchProtectedData').addEventListener('click', async () => {
    if (!token) {
        document.getElementById('protectedData').textContent = 'Пожалуйста, войдите в систему';
        return;
    }
